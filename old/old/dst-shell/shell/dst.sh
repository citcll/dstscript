...
#===============================================================================
#	System Required: Debian 6+/Ubuntu 14.04+(CentOS有个问题暂无法解决，后续会更新吧！)
#	Description: Install and manager the Don't Starve Together Dedicated Server
#	Version: 2.0.0 （我决定认真做个2.0版本，认真脸！！！）
#	Author: GoforDream
#	Blog: https://blog.wqlin.com/dst-shell.html
#===============================================================================
...
steamcmd_file="${steamcmd_folder}/steamcmd.sh"
config_folder="${filepath}/data"
...
check_root(){
	[[ $EUID == 0 ]] && echo -e "${Tip} 当前账号为ROOT，存在安全隐患，如你使用非本人提供的来源获取的脚本导致系统出现问题，本人概不负责！"
}
check_sys(){
...
DST_installation_status(){
	...
	[[ ! -e ${DST_folder} ]] && echo -e "${Error} 没有发现 ShadowsocksR 文件夹，请检查 !" && exit 1
}
# 读取 配置信息
Get_IP(){
...
DST_link_qr(){
	DSTprotocol=$(echo ${protocol} | sed 's/_compatible//g')
	DSTobfs=$(echo ${obfs} | sed 's/_compatible//g')
	DSTPWDbase64=$(urlsafe_base64 "${password}")
	DSTbase64=$(urlsafe_base64 "${ip}:${port}:${DSTprotocol}:${method}:${DSTobfs}:${DSTPWDbase64}")
	DSTurl="DST://${DSTbase64}"
	DSTQRcode="http://doub.pw/qr/qr.php?text=${DSTurl}"
	DST_link=" DST   链接 : ${Red_font_prefix}${DSTurl}${Font_color_suffix} \n DST 二维码 : ${Red_font_prefix}${DSTQRcode}${Font_color_suffix} \n "
}
ss_DST_determine(){
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 2333):" DST_port
	[[ -z "$DST_port" ]] && DST_port="2333"
	expr ${DST_port} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${DST_port} -ge 1 ]] && [[ ${DST_port} -le 65535 ]]; then
			echo && echo ${Separator_1} && echo -e "	端口 : ${Green_font_prefix}${DST_port}${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: doub.io):" DST_password
	[[ -z "${DST_password}" ]] && DST_password="doub.io"
	echo && echo ${Separator_1} && echo -e "	密码 : ${Green_font_prefix}${DST_password}${Font_color_suffix}" && echo ${Separator_1} && echo
}
Set_config_method(){
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	stty erase '^H' && read -p "(默认: 5. aes-128-ctr):" DST_method
	[[ -z "${DST_method}" ]] && DST_method="5"
	if [[ ${DST_method} == "1" ]]; then
		DST_method="none"
	elif [[ ${DST_method} == "2" ]]; then
		DST_method="rc4"
	elif [[ ${DST_method} == "3" ]]; then
		DST_method="rc4-md5"
	elif [[ ${DST_method} == "4" ]]; then
		DST_method="rc4-md5-6"
	elif [[ ${DST_method} == "5" ]]; then
		DST_method="aes-128-ctr"
	elif [[ ${DST_method} == "6" ]]; then
		DST_method="aes-192-ctr"
	elif [[ ${DST_method} == "7" ]]; then
		DST_method="aes-256-ctr"
	elif [[ ${DST_method} == "8" ]]; then
		DST_method="aes-128-cfb"
	elif [[ ${DST_method} == "9" ]]; then
		DST_method="aes-192-cfb"
	elif [[ ${DST_method} == "10" ]]; then
		DST_method="aes-256-cfb"
	elif [[ ${DST_method} == "11" ]]; then
		DST_method="aes-128-cfb8"
	elif [[ ${DST_method} == "12" ]]; then
		DST_method="aes-192-cfb8"
	elif [[ ${DST_method} == "13" ]]; then
		DST_method="aes-256-cfb8"
	elif [[ ${DST_method} == "14" ]]; then
		DST_method="salsa20"
	elif [[ ${DST_method} == "15" ]]; then
		DST_method="chacha20"
	elif [[ ${DST_method} == "16" ]]; then
		DST_method="chacha20-ietf"
	else
		DST_method="aes-128-ctr"
	fi
	echo && echo ${Separator_1} && echo -e "	加密 : ${Green_font_prefix}${DST_method}${Font_color_suffix}" && echo ${Separator_1} && echo
}
Set_config_protocol(){
 ...
 ${Tip} 如果使用 auth_chain_a 协议，请加密方式选择 none，混淆随意(建议 plain)" && echo
	stty erase '^H' && read -p "(默认: 2. auth_sha1_v4):" DST_protocol
	[[ -z "${DST_protocol}" ]] && DST_protocol="2"
	if [[ ${DST_protocol} == "1" ]]; then
		DST_protocol="origin"
	elif [[ ${DST_protocol} == "2" ]]; then
		DST_protocol="auth_sha1_v4"
	elif [[ ${DST_protocol} == "3" ]]; then
		DST_protocol="auth_aes128_md5"
	elif [[ ${DST_protocol} == "4" ]]; then
		DST_protocol="auth_aes128_sha1"
	elif [[ ${DST_protocol} == "5" ]]; then
		DST_protocol="auth_chain_a"
	elif [[ ${DST_protocol} == "6" ]]; then
		DST_protocol="auth_chain_b"
	else
		DST_protocol="auth_sha1_v4"
	fi
	echo && echo ${Separator_1} && echo -e "	协议 : ${Green_font_prefix}${DST_protocol}${Font_color_suffix}" && echo ${Separator_1} && echo
	if [[ ${DST_protocol} != "origin" ]]; then
		if [[ ${DST_protocol} == "auth_sha1_v4" ]]; then
			stty erase '^H' && read -p "是否设置 协议插件兼容原版(_compatible)？[Y/n]" DST_protocol_yn
			[[ -z "${DST_protocol_yn}" ]] && DST_protocol_yn="y"
			[[ $DST_protocol_yn == [Yy] ]] && DST_protocol=${DST_protocol}"_compatible"
			echo
		fi
	fi
}
Set_config_obfs(){
 ...
 ${Tip} 如果使用 ShadowsocksR 加速游戏，请选择 混淆兼容原版或 plain 混淆，然后客户端选择 plain，否则会增加延迟 !" && echo
	stty erase '^H' && read -p "(默认: 5. tls1.2_ticket_auth):" DST_obfs
	[[ -z "${DST_obfs}" ]] && DST_obfs="5"
	if [[ ${DST_obfs} == "1" ]]; then
		DST_obfs="plain"
	elif [[ ${DST_obfs} == "2" ]]; then
		DST_obfs="http_simple"
	elif [[ ${DST_obfs} == "3" ]]; then
		DST_obfs="http_post"
	elif [[ ${DST_obfs} == "4" ]]; then
		DST_obfs="random_head"
	elif [[ ${DST_obfs} == "5" ]]; then
		DST_obfs="tls1.2_ticket_auth"
	else
		DST_obfs="tls1.2_ticket_auth"
	fi
	echo && echo ${Separator_1} && echo -e "	混淆 : ${Green_font_prefix}${DST_obfs}${Font_color_suffix}" && echo ${Separator_1} && echo
	if [[ ${DST_obfs} != "plain" ]]; then
			stty erase '^H' && read -p "是否设置 混淆插件兼容原版(_compatible)？[Y/n]" DST_obfs_yn
			[[ -z "${DST_obfs_yn}" ]] && DST_obfs_yn="y"
			[[ $DST_obfs_yn == [Yy] ]] && DST_obfs=${DST_obfs}"_compatible"
			echo
	fi
}
Set_config_protocol_param(){
	...
	stty erase '^H' && read -p "(默认: 无限):" DST_protocol_param
	[[ -z "$DST_protocol_param" ]] && DST_protocol_param="" && echo && break
	expr ${DST_protocol_param} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${DST_protocol_param} -ge 1 ]] && [[ ${DST_protocol_param} -le 9999 ]]; then
			echo && echo ${Separator_1} && echo -e "	设备数限制 : ${Green_font_prefix}${DST_protocol_param}${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
...
Set_config_speed_limit_per_con(){
	...
	stty erase '^H' && read -p "(默认: 无限):" DST_speed_limit_per_con
	[[ -z "$DST_speed_limit_per_con" ]] && DST_speed_limit_per_con=0 && echo && break
	expr ${DST_speed_limit_per_con} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${DST_speed_limit_per_con} -ge 1 ]] && [[ ${DST_speed_limit_per_con} -le 131072 ]]; then
			echo && echo ${Separator_1} && echo -e "	单线程限速 : ${Green_font_prefix}${DST_speed_limit_per_con} KB/S${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
...
Set_config_speed_limit_per_user(){
	...
	stty erase '^H' && read -p "(默认: 无限):" DST_speed_limit_per_user
	[[ -z "$DST_speed_limit_per_user" ]] && DST_speed_limit_per_user=0 && echo && break
	expr ${DST_speed_limit_per_user} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${DST_speed_limit_per_user} -ge 1 ]] && [[ ${DST_speed_limit_per_user} -le 131072 ]]; then
			echo && echo ${Separator_1} && echo -e "	端口总限速 : ${Green_font_prefix}${DST_speed_limit_per_user} KB/S${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 请输入正确的数字(1-131072)"
		fi
...
# 修改 配置信息
Modify_config_port(){
	sed -i 's/"server_port": '"$(echo ${port})"'/"server_port": '"$(echo ${DST_port})"'/g' ${config_user_file}
}
Modify_config_password(){
	sed -i 's/"password": "'"$(echo ${password})"'"/"password": "'"$(echo ${DST_password})"'"/g' ${config_user_file}
}
Modify_config_method(){
	sed -i 's/"method": "'"$(echo ${method})"'"/"method": "'"$(echo ${DST_method})"'"/g' ${config_user_file}
}
Modify_config_protocol(){
	sed -i 's/"protocol": "'"$(echo ${protocol})"'"/"protocol": "'"$(echo ${DST_protocol})"'"/g' ${config_user_file}
}
Modify_config_obfs(){
	sed -i 's/"obfs": "'"$(echo ${obfs})"'"/"obfs": "'"$(echo ${DST_obfs})"'"/g' ${config_user_file}
}
Modify_config_protocol_param(){
	sed -i 's/"protocol_param": "'"$(echo ${protocol_param})"'"/"protocol_param": "'"$(echo ${DST_protocol_param})"'"/g' ${config_user_file}
}
Modify_config_speed_limit_per_con(){
	sed -i 's/"speed_limit_per_con": '"$(echo ${speed_limit_per_con})"'/"speed_limit_per_con": '"$(echo ${DST_speed_limit_per_con})"'/g' ${config_user_file}
}
Modify_config_speed_limit_per_user(){
	sed -i 's/"speed_limit_per_user": '"$(echo ${speed_limit_per_user})"'/"speed_limit_per_user": '"$(echo ${DST_speed_limit_per_user})"'/g' ${config_user_file}
}
Modify_config_connect_verbose_info(){
	sed -i 's/"connect_verbose_info": '"$(echo ${connect_verbose_info})"'/"connect_verbose_info": '"$(echo ${DST_connect_verbose_info})"'/g' ${config_user_file}
}
Modify_config_all(){
	Modify_config_port
	Modify_config_password
...
Modify_config_port_many(){
	sed -i 's/"'"$(echo ${port})"'":/"'"$(echo ${DST_port})"'":/g' ${config_user_file}
}
Modify_config_password_many(){
	sed -i 's/"'"$(echo ${password})"'"/"'"$(echo ${DST_password})"'"/g' ${config_user_file}
}
# 写入 配置信息
Write_configuration(){
	cat > ${config_user_file}<<-EOF
{
    ...
    "protocol_param": "${DST_protocol_param}",
    "obfs": "${DST_obfs}",
    "obfs_param": "",
    "speed_limit_per_con": ${DST_speed_limit_per_con},
    "speed_limit_per_user": ${DST_speed_limit_per_user},
...
Download_DST(){
	...
	#[[ ! -e ${DST_folder} ]] && echo -e "${Error} ShadowsocksR服务端 下载失败 !" && exit 1
...
Service_DST(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/other/DST_centos -O /etc/init.d/DST; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/DST
		chkconfig --add DST
		chkconfig DST on
	else
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/other/DST_debian -O /etc/init.d/DST; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/DST
		update-rc.d -f DST defaults
	fi
...
Install_DST(){
	...
	[[ -e ${DST_folder} ]] && echo -e "${Error} ShadowsocksR 文件夹已存在，请检查( 如安装失败或者存在旧版本，请先卸载 ) !" && exit 1
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 显示 IP+IP归属地 格式" && echo
	stty erase '^H' && read -p "(默认: 1):" DST_connection_info
	[[ -z "${DST_connection_info}" ]] && DST_connection_info="1"
	if [[ ${DST_connection_info} == "1" ]]; then
		View_user_connection_info_1 ""
	elif [[ ${DST_connection_info} == "2" ]]; then
...
Modify_Config(){
 ...
 ${Green_font_prefix}9.${Font_color_suffix} 修改 全部配置" && echo
		stty erase '^H' && read -p "(默认: 取消):" DST_modify
		[[ -z "${DST_modify}" ]] && echo "已取消..." && exit 1
		Get_User
		if [[ ${DST_modify} == "1" ]]; then
			Set_config_port
			Modify_config_port
			Add_iptables
			Del_iptables
			Save_iptables
		elif [[ ${DST_modify} == "2" ]]; then
			Set_config_password
			Modify_config_password
		elif [[ ${DST_modify} == "3" ]]; then
			Set_config_method
			Modify_config_method
		elif [[ ${DST_modify} == "4" ]]; then
			Set_config_protocol
			Modify_config_protocol
		elif [[ ${DST_modify} == "5" ]]; then
			Set_config_obfs
			Modify_config_obfs
		elif [[ ${DST_modify} == "6" ]]; then
			Set_config_protocol_param
			Modify_config_protocol_param
		elif [[ ${DST_modify} == "7" ]]; then
			Set_config_speed_limit_per_con
			Modify_config_speed_limit_per_con
		elif [[ ${DST_modify} == "8" ]]; then
			Set_config_speed_limit_per_user
			Modify_config_speed_limit_per_user
		elif [[ ${DST_modify} == "9" ]]; then
			Set_config_all
			Modify_config_all
		else
...
Add_multi_port_user(){
	Set_config_port
	Set_config_password
	sed -i "8 i \"        \"${DST_port}\":\"${DST_password}\"," ${config_user_file}
	sed -i "8s/^\"//" ${config_user_file}
	Add_iptables
	Save_iptables
	echo -e "${Info} 多端口用户添加完成 ${Green_font_prefix}[端口: ${DST_port} , 密码: ${DST_password}]${Font_color_suffix} "
}
# 修改 多端口用户配置
Modify_multi_port_user(){
	List_multi_port_user
	...
	if [[ ! -z "${del_user}" ]]; then
		...
		Set_config_port
		Set_config_password
		sed -i 's/"'$(echo ${port})'":"'$(echo ${password})'"/"'$(echo ${DST_port})'":"'$(echo ${DST_password})'"/g' ${config_user_file}
		Del_iptables
		Add_iptables
		Save_iptables
		echo -e "${Inof} 多端口用户修改完成 ${Green_font_prefix}[旧: ${modify_user_port}  ${password} , 新: ${DST_port}  ${DST_password}]${Font_color_suffix} "
	else
...
Manually_Modify_Config(){
	...
	vi ${config_user_file}
	if [[ -z "${now_mode}" ]]; then
		DST_port=`${jq_file} '.server_port' ${config_user_file}`
		Del_iptables
		Add_iptables
	fi
	Restart_DST
...
————————————
  ...
  ——说明：DST默认只输出错误日志，此项可切换为输出详细的访问日志" && echo
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://cloud.wqlin.com/Bash/dst.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="cloud"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/GoforDance/DirectoryLister/master/Bash/dst.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			if [[ $sh_new_type == "cloud" ]]; then
				wget -N --no-check-certificate https://cloud.wqlin.com/Bash/dst.sh && chmod +x dst.sh
			else
				wget -N --no-check-certificate https://raw.githubusercontent.com/GoforDance/DirectoryLister/master/Bash/dst.sh && chmod +x dst.sh
			fi
